var dynamicTemplate = loadTemplate('assets/templates/staff/allotedDynamic.html');

var allotedDynamicView = Backbone.View.extend({
	initialize:function(){},
	el:'#assign',
	render:function(){
		this.$el.empty();
	...
	init:function(objEntityId,objEntityTypeId,editType,companyName){
		...
		$("#dynamicType1").append(str);
		$('#titlee1').html(decodeURIComponent(companyName));
...
var allotedDynamicViewObj = new allotedDynamicView();
...
function wordDynamic(dynamicId, uId, uName, aobj, cnt, f) {
    ...
    var data = {
        ...
        "objEntityTypeId" : (allotedDynamicViewObj.objEntityTypeId == 'operatelog' ? '07' : '06')
    ...
    new DataDynamic({
        ...
        ajax : {
            url : allotedDynamicViewObj.urlMap + '/word/page',
            data : data
        }
...
function addWord(dynamicId, uId, uName, f) {
    ...
    ajax({
        url : allotedDynamicViewObj.urlMap + "/word/reply",
        data : data,
