//加载并初始化模板对象
var dynamicEditMainTemplate = loadTemplate("assets/templates/customer/dynamicEdit.html");
var objEntityTypeId = '';
var objEntityId= '';
var editType = '';
var urlmap = '';
...
var dynamicEditModelMainView = Backbone.View.extend({
    ...
    template : dynamicEditMainTemplate,
    render : function() {
        this.$el.empty();
    ...
    events : {
        'click #wordDynamic' : 'wordDynamic'
    },
    getDynamicData : function(objEntityIds, companyName, objEntityTypeIds, editType, cusotm) {
        //alert('加载数据...');
        var self = this;
        self.render();
        ...
        case 'operatelog':
            //客户动态日志
            urlmap = '/custom';
            self.getOperatelog(objEntityId, editType, urlmap);
            break;
        case '01':
            //营销数据
            urlmap = '/marketing';
            self.getDynamic(objEntityId, objEntityTypeId, editType,custom);
            break;
        case '02':
            //线索
            urlmap = '/clue';
    ...
    getOperatelog : function(objEntityId, editType) {
        var data = {
            "customId" : objEntityId,
        };
        new DataDynamic({
            id : '#list',
            paging : true,
            ...
            editType : editType,
            ajax : {
                url : urlmap + '/operatelog/page',
                data : data
            }
        });
    }
});
var dynamicEditModelMainViewInstance = new dynamicEditModelMainView();
...
function wordReply(dynamicId, uId, uName) {
    var str = '<li class="list-group-item bg-primary"><form class="form-horizontal"><div class="form-group"><div class="col-sm-12"><textarea class="form-control" placeholder="回复' + decodeURIComponent(uName) + '：" cols="30" rows="3" id="word' + dynamicId + '" maxlength="200" title="回复内容字符个数不能超过200个!"></textarea></div></div><div class="row"><div class="col-sm-10"></div><div class="col-sm-2 "><button class="btn btn-info" type="button" id="submit" onclick="addWord(' + dynamicId + ',\'' + uId + '\',\'' + (uName) + '\',1)" style="float:right;">回复</button></div></div></form><div class="" ></li>'
...
function DataDynamic(options) {
    ...
    var loadContent = _this.loadContent = function(pageNo) {
        ...
        if (_this.type == 'wordlist') {
            //评论
            var list = '<form class="form-horizontal"><div class="form-group"><div class="col-sm-12"><textarea class="form-control" placeholder="请填写评论" cols="30" rows="3" id="wordContent' + pageId.split('dynamic')[1] + '" maxlength="200" title="评论字符个数不能超过200个!"></textarea></div></div><div class="row"><div class="col-sm-11"></div><div class="col-sm-1"><button class="btn btn-info" type="button" id="submit" style="float:right;" onclick="addWord(' + pageId.split('dynamic')[1] + ',\'\',\'\')">评论</button></div></div></form><div class="" ><div class="panel-heading"></div><div class="panel-heading"></div>'
            ths.push(list);
        }
