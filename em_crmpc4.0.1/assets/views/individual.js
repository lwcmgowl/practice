//加载并初始化模板对象
var individualTemplate = loadTemplate("assets/templates/staff/individual.html");
var personalTemplate = loadTemplate("assets/templates/staff/perPartnerDetail.html");
var picurllist = "";
var objId = "";
//列表容器VIEW
var marketListView = Backbone.View.extend({
    ...
    events : {
        'click #searchmy' : function() {
            this.load();
        },
        // 'click #add' : 'add',
        'click #import' : 'import',
        'click #expedm' : 'expedm',
        'click .field_edit' : "fieledit",
        "click #maskIndividual":function(){
            this.hideDetail();
        }
    },
        ...
        new DataTable({
            ...
            columnDefs : [{
                ...
                render : function(i, j, c) {
                    var html = "<a href='javascript:;' onclick='individualViewInstance.partnerDetail(\"" + c.id + "\")' title=" + c.contactName + ">" + c.contactName + "</a>";
                    // var html = "<a href='#perPartnerDetail/" + c.id + "'>" + c.contactName + "</a>";
                    return html;
                }
            }],
    ...
    exportFile:function(){
        var data = {
            ...
            individualService.exportFile(data, url)
    },
    partnerDetail:function(id){
        var self=this;
        var pushRight = document.getElementById( 'pushRight' );
            classie.toggle( pushRight, 'cbp-spmenu-open' );
             $("#maskIndividual").css("height",$(document).height());     
                $("#maskIndividual").css("width",$(document).width());     
                $("#maskIndividual").show();
            ...
            success : function(cols, resp, options) {
                ...
                if (attachment) {
                    ...
                    for (var p in piclist) {
                        ...
                        $("#uppictrs").append("<td  id='pic" + piclist[p] + "'><img src='" + urldownshow + "' width='60px'  height='60px'/> <a href='javascript:;' onclick='individualViewInstance.editdelpic(\"" + piclist[p] + "\")'>删除</a></td>");
    ...
    edit : function() {
        ...
        individualViewInstance.model.clear("id");
        individualViewInstance.model.set({
            id : objId
        });
        individualViewInstance.model.save({
            param : param
        }, {
            success : function(cols, resp, options) {
                $.success("编辑成功", null, null, function() {
                    individualViewInstance.load();
                });
            },
            error : function(cols, resp, options) {
                if (resp == "001") {
        ...
        if (attcId == '.jpg' || attcId == '.gif' || attcId == '.png') {
            ...
            $("#imgForm").ajaxSubmit({
                ...
                success : function(data) {
                    ...
                    $("#uppictr").append("<td id='pic" + data.msg.objectId + "'><img src='" + urldownshow + "' width='60px'  height='60px'/> <a href='javascript:;' onclick='individualViewInstance.delpic(\"" + data.msg.objectId + "\")'>删除</a></td>");
    ...
    editpreview : function(urlVal) {
        ...
        if (attcId == '.jpg' || attcId == '.gif' || attcId == '.png') {
            ...
            $("#editimgForm").ajaxSubmit({
                ...
                success : function(data) {
                    var urldownshow = picviewPath + "/" + data.msg.objectId;
                    $("#uppictrs").append("<td id='pic" + data.msg.objectId + "'><img src='" + urldownshow + "' width='60px'  height='60px'/><a href='javascript:;' onclick='individualViewInstance.editdelpic(\"" + data.msg.objectId + "\")'>删除</a> </td>");
                    picurllist += (data.msg.objectId + ",");
        ...
        bootbox.dialog({
            ...
            buttons : {
                ok : {
                    ...
                    callback : function() {
                        $('#personalPartner').bootstrapValidator('validate');
                        var loginId = appcanUserInfo.userId;
                        var contactName = $("#pcontactName").val();
                        ...
                        if (!isDefine(region) || region.indexOf("选择") > 0) {
                            ...
                            $("#__region").parent().removeClass("has-error");
                        }
            ...
            complete : function() {
                ...
                $('#personalPartner').bootstrapValidator({
                    message : '无效的数值!',
                    feedbackIcons : {
...
function preview(urlVal) {
    individualViewInstance.preview(urlVal);
}
