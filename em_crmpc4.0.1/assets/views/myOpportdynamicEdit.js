var dynamicTemplate = loadTemplate('assets/templates/staff/myReportdynamicEdit.html');
var objEntityTypeId = '';
var objEntityId= '';
var editType = '';
var urlmap = '';
...
function wordReply(dynamicId, uId, uName) {
    var str = '<li class="list-group-item bg-primary"><form class="form-horizontal"><div class="form-group"><div class="col-sm-12"><textarea class="form-control" placeholder="回复' + decodeURIComponent(uName) + '：" cols="30" rows="3" id="word' + dynamicId + '" maxlength="200" title="回复内容字符数不能超过200个!"></textarea></div></div><div class="row"><div class="col-sm-10"></div><div class="col-sm-2 "><button class="btn btn-info" type="button" id="submit" onclick="addWord(' + dynamicId + ',\'' + uId + '\',\'' + (uName) + '\',1)" style="float:right;">回复</button></div></div></form><div class="" ></li>'
...
function DataDynamic(options) {
    ...
    var initOption = function() {
        ...
        $('#page' + pageId).remove();
        $table.parent().append('<div id="page' + pageId + '">' + '<div class="dataTables_info" id="datatable_info" role="status" aria-live="polite">共 ' + _this.total + ' 条数据</div>' + '<nav class="pagination-poss pages' + pageId + '"></nav>' + '</div>');
        if (_this.total == 0) {
            $('.pagination-poss').css('display', 'none');
        }
    };
    ...
    var loadContent = _this.loadContent = function(pageNo) {
        ...
        if (_this.type == 'wordlist') {
            //评论
            var list = '<form class="form-horizontal"><div class="form-group"><div class="col-sm-12"><textarea class="form-control" placeholder="请填写评论" cols="30" rows="3" id="wordContent' + pageId.split('dynamic')[1] + '" maxlength="200" title="评论内容字符个数不能超过200个!"></textarea></div></div><div class="row"><div class="col-sm-11"></div><div class="col-sm-1"><button class="btn btn-info" type="button" id="submit" style="float:right;" onclick="addWord(' + pageId.split('dynamic')[1] + ',\'\',\'\')">评论</button></div></div></form><div class="" ><div class="panel-heading"></div><div class="panel-heading"></div>'
            ths.push(list);
        }
                    ...
                    success : function(data) {
                        try {
                            if (data.status == "000") {
                                ...
                                page.defalut.total = data.msg.count;
                                initOption();
                                page.$wrapper = $('.pages' + _this.pageId);
                                page.redner();
                                _this.loadContent(1);
                                if (_this.type == 'wordlist') {
                                    _this.$table.parent().removeClass('hide');
