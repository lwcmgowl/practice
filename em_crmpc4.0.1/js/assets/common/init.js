...
function Pagination(option, mark) {
    ...
    this.redner = function() {
        ...
        if (_this.defalut.prev) {
            if (_this.defalut.currNo == 1) {
                html += '<li class="disabled">' + '<a href="javascript:;" aria-label="Previous">' + '<span aria-hidden="true">&laquo;</span>' + '</a>' + '</li>';
            } else {
        ...
        if (_this.defalut.next) {
            if (_this.defalut.currNo == _this.defalut.total) {
                html += '<li class="disabled">' + '<a href="javascript:;" aria-label="Next">' + '<span aria-hidden="true">&raquo;</span>' + '</a>' + '</li>';
            } else {
    ...
    //分页跳转
    this.go = function(pageNo,markdyna) {
        pageNo = +(pageNo || 1) || 1;
        if (mark != 1&&markdyna!=1) {
...
function DataTable(options, mark) {
    ...
    var createBody = function(pageNo) {
        ...
        for (var i = begin; i < end; i++) {
            ...
            tds.push('<tr>');
            for (var j = 0,
                length = fields.length; j < length; j++) {
    ...
    $('.pagination-pos').remove();
    $('.' + options.el + '_info').remove();
    initDefault(options);
    //初始化分页
    var cnt = Math.ceil(options.total / options.pageSize);
    var page = new Pagination({
        total : cnt,
        wrapper : '.' + options.el + "_pos",
        callback : request,
        dataTableCb : options.dataTableCb
    }, mark);
